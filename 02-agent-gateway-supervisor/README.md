...
| `polkit/49-hermes-hermes.rules` | lets user `hermes` run `systemctl restart/start` on the two units. Without it: `Access denied` (sudo is unusable under `NoNewPrivileges`). Install as root into `/etc/polkit-1/rules.d/`. |
...
3. **An "up" unit can still be dead.** `systemctl is-active` is not enough: the failure signature was in log tails ("session storage could not be written") while the process ran fine. Watch heartbeats and fresh write-failure markers, not only process state.
