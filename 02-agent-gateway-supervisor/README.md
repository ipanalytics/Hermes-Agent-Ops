...
| `polkit/49-hermes-agent.rules` | lets user `hermes` run `systemctl restart/start` on the two units. Without it: `Access denied` (sudo is unusable under `NoNewPrivileges`). Install as root into `/etc/polkit-1/rules.d/`. |
