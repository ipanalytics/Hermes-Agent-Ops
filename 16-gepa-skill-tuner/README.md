...
4. **No fake improvements.** If nothing beats the seed, the tool says so
   (`No prompt change survived validation`) and writes no diff. My shell-safety example does
...
2. **Label.** Keep only rows where the outcome is verifiable (the skill really ran, the task
   succeeded). Drop the ambiguous ones instead of guessing: 30 of 70 unique rows in my set were
