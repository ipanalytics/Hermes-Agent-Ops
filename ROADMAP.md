...
5. **v0.4 — tests & CI.** Python tools (01, 02, 07, 09, 10, 14) get test suites; linter + exporter wired into a GitHub Action that self-checks this repo's own prompts and watch-table. **Started:** `15-agent-tool-guardrails` ships with two suites (22 + 16 checks) that exercise the real hook contract and a scratch-repo lane lifecycle.
...
7. **v0.5 — optimization loop.** `16-gepa-skill-tuner` extends from one example into recipes for the surfaces with a measurable failure rate (cron prompts, skill sections, routing prompts). ✅ module shipped; the routing-prompt recipe (harvest → label → dedupe → small budget → ship the diff) is documented in the module README.
8. **v0.6 — community.** First external contributions: new persona packs, new cron recipes, new incident autopsies (attributed or anonymized).
